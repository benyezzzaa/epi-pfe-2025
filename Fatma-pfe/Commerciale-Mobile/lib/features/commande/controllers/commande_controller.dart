...
Future<void> createCommande(int clientId, Map<int, int> cart) async {
  try {
    ...
    // Génération d'un numéro de commande professionnel
    final now = DateTime.now();
    final year = now.year.toString().substring(2); // 2025 -> 25
    final month = now.month.toString().padLeft(2, '0'); // 3 -> 03
    final day = now.day.toString().padLeft(2, '0'); // 17 -> 17
    final hour = now.hour.toString().padLeft(2, '0'); // 14 -> 14
    final minute = now.minute.toString().padLeft(2, '0'); // 30 -> 30
    final second = now.second.toString().padLeft(2, '0'); // 45 -> 45
    
    // Format: CMD-YYMMDD-HHMMSS (ex: CMD-250317-143045)
    final numeroCommande = 'CMD-$year$month$day-$hour$minute$second';

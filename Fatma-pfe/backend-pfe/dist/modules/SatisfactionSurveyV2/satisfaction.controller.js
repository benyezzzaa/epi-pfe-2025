"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SatisfactionController = void 0;
const common_1 = require("@nestjs/common");
const satisfaction_service_1 = require("./satisfaction.service");
const create_survey_dto_1 = require("./dto/create-survey.dto");
const create_response_dto_1 = require("./dto/create-response.dto");
let SatisfactionController = class SatisfactionController {
    satisfactionService;
    constructor(satisfactionService) {
        this.satisfactionService = satisfactionService;
    }
    createSurvey(dto) {
        return this.satisfactionService.createSurvey(dto);
    }
    getSurveys() {
        return this.satisfactionService.getSurveys();
    }
    getSurvey(id) {
        return this.satisfactionService.getSurveyById(+id);
    }
    async getSurveyQRCode(id, baseUrl) {
        return {
            qrCode: await this.satisfactionService.generateSurveyQRCode(+id, baseUrl),
        };
    }
    async getSurveyPdf(id, res) {
        return this.satisfactionService.generateSurveyPdf(+id, res);
    }
    createResponse(dto) {
        return this.satisfactionService.createResponse(dto);
    }
};
exports.SatisfactionController = SatisfactionController;
__decorate([
    (0, common_1.Post)('survey'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_survey_dto_1.CreateSurveyDto]),
    __metadata("design:returntype", void 0)
], SatisfactionController.prototype, "createSurvey", null);
__decorate([
    (0, common_1.Get)('surveys'),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", []),
    __metadata("design:returntype", void 0)
], SatisfactionController.prototype, "getSurveys", null);
__decorate([
    (0, common_1.Get)('survey/:id'),
    __param(0, (0, common_1.Param)('id')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number]),
    __metadata("design:returntype", void 0)
], SatisfactionController.prototype, "getSurvey", null);
__decorate([
    (0, common_1.Get)('survey/:id/qrcode'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Query)('baseUrl')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, String]),
    __metadata("design:returntype", Promise)
], SatisfactionController.prototype, "getSurveyQRCode", null);
__decorate([
    (0, common_1.Get)('survey/:id/pdf'),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Number, Object]),
    __metadata("design:returntype", Promise)
], SatisfactionController.prototype, "getSurveyPdf", null);
__decorate([
    (0, common_1.Post)('response'),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [create_response_dto_1.CreateResponseDto]),
    __metadata("design:returntype", void 0)
], SatisfactionController.prototype, "createResponse", null);
exports.SatisfactionController = SatisfactionController = __decorate([
    (0, common_1.Controller)('satisfaction'),
    __metadata("design:paramtypes", [satisfaction_service_1.SatisfactionService])
], SatisfactionController);
//# sourceMappingURL=satisfaction.controller.js.map
...
class _CataloguePageState extends State<CataloguePage>
  ...
  Future<void> loadProduits() async {
    try {
      ...
      // Filtrer les produits pour exclure ceux avec des valeurs null ou manquantes
      final produitsFiltres = produits.where((p) {
        return p['nom'] != null && 
               p['nom'].toString().isNotEmpty &&
               p['categorie'] != null &&
               p['categorie']['nom'] != null &&
               p['categorie']['nom'].toString().isNotEmpty;
      }).toList();
      
      final Set<String> cats = produitsFiltres.map<String>((p) => p['categorie']['nom'] as String).toSet();
      setState(() {
        allProduits = produitsFiltres;
        categories = ['Tous', ...cats];
        displayedProduits = List.from(produitsFiltres);
        isLoading = false;
      });
      
      print("📦 Produits chargés: ${produitsFiltres.length}/${produits.length} (après filtrage)");
    } catch (e) {
                                        ...
                                        child: Padding(
                                          ...
                                          child: Column(
                                            ...
                                            children: [
                                              ...
                                              if (p['unite'] != null && p['unite']['nom'] != null && p['unite']['nom'].toString().isNotEmpty)
                                                Text(
                                                  p['unite']['nom'],
                                                  style: TextStyle(
